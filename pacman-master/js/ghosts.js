...
//* declarar al fantasma blinky
var GHOST_BLINKY_CANVAS_CONTEXT = null;  //* blinky en el canvas
var GHOST_BLINKY_POSITION_X = 276; //* posicion en x
var GHOST_BLINKY_POSITION_Y = 204;//* posicion en y
var GHOST_BLINKY_DIRECTION = 1; //* la direccion
var GHOST_BLINKY_COLOR = "#ed1b24"; //* color 
var GHOST_BLINKY_MOVING_TIMER = -1; //* tiempo en el que se mueve en el tablero
var GHOST_BLINKY_MOVING = false; //* si se esta moviendo o no
var GHOST_BLINKY_BODY_STATE = 0; //*estado del fantasma en el body
var GHOST_BLINKY_STATE = 0; //* estado del fantasma
var GHOST_BLINKY_EAT_TIMER = null; //* tiempo  comido
var GHOST_BLINKY_AFFRAID_TIMER = null;//* tiempo en el que esta austado
var GHOST_BLINKY_AFFRAID_STATE = 0; //* estado asustado
var GHOST_BLINKY_TUNNEL = false; //* blinky en los caminos

//* declarar al fantasma pinky
var GHOST_PINKY_CANVAS_CONTEXT = null;   //* pinky en el canvas
var GHOST_PINKY_POSITION_X = 276; //* posicion en x
var GHOST_PINKY_POSITION_Y = 258; //* posicion en y
var GHOST_PINKY_DIRECTION = 2; //* la direccion
var GHOST_PINKY_COLOR = "#feaec9"; //* color
var GHOST_PINKY_MOVING_TIMER = -1;//* tiempo en el que se mueve en el tablero
var GHOST_PINKY_MOVING = false; //* si se esta moviendo o no
var GHOST_PINKY_BODY_STATE = 1;  //*estado del fantasma en el body
var GHOST_PINKY_STATE = 0; //* estado del fantasma
var GHOST_PINKY_EAT_TIMER = null; //* tiempo  comido
var GHOST_PINKY_AFFRAID_TIMER = null; //* tiempo en el que esta austado
var GHOST_PINKY_AFFRAID_STATE = 0; //* estado asustado
var GHOST_PINKY_TUNNEL = false; //* pinky en la caja

var GHOST_INKY_CANVAS_CONTEXT = null;  //* inky en el canvas
var GHOST_INKY_POSITION_X = 238; //* posicion en x
var GHOST_INKY_POSITION_Y = 258; //* posicion en y
var GHOST_INKY_DIRECTION = 3; //* la direccion
var GHOST_INKY_COLOR = "#4adecb"; //* color
var GHOST_INKY_MOVING_TIMER = -1; //* tiempo en el que se mueve en el tablero
var GHOST_INKY_MOVING = false; //* si se esta moviendo o no
var GHOST_INKY_BODY_STATE = 2;  //*estado del fantasma en el body
var GHOST_INKY_STATE = 0; //* estado del fantasma
var GHOST_INKY_EAT_TIMER = null; //* tiempo  comido
var GHOST_INKY_AFFRAID_TIMER = null; //* tiempo en el que esta austado
var GHOST_INKY_AFFRAID_STATE = 0; //* estado asustado
var GHOST_INKY_TUNNEL = false; //* inky en la caja

var GHOST_CLYDE_CANVAS_CONTEXT = null;  //* clyde en el canvas
var GHOST_CLYDE_POSITION_X = 314; //* posicion en x
var GHOST_CLYDE_POSITION_Y = 258; //* posicion en y
var GHOST_CLYDE_DIRECTION = 4; //* la direccion
var GHOST_CLYDE_COLOR = "#f99c00"; //* color
var GHOST_CLYDE_MOVING_TIMER = -1;//* tiempo en el que se mueve en el tablero
var GHOST_CLYDE_MOVING = false; //* si se esta moviendo o no
var GHOST_CLYDE_BODY_STATE = 3;  //*estado del fantasma en el body
var GHOST_CLYDE_STATE = 0; //* estado del fantasma
var GHOST_CLYDE_EAT_TIMER = null; //* tiempo  comido
var GHOST_CLYDE_AFFRAID_TIMER = null; //* tiempo en el que esta austado
var GHOST_CLYDE_AFFRAID_STATE = 0; //* estado asustado
var GHOST_CLYDE_TUNNEL = false; //* clyde en la caja

var GHOST_AFFRAID_COLOR = "#2d3eff";  //* color de los fantasmas asustados
var GHOST_AFFRAID_FINISH_COLOR = "#fff"; //* color cuando va a terminar su estado de asustado
var GHOST_POSITION_STEP = 2; //* pasos del fantasma
var GHOST_MOVING_SPEED = 15; //* velocidad de movimiento de los fantasmas
var GHOST_TUNNEL_MOVING_SPEED = 35; //* movimiento en el tunel
var GHOST_AFFRAID_MOVING_SPEED = 40; //* aceleracion de movimiento cuando estan asustados (entre mas alto el numero menor su aceleracion)
var GHOST_EAT_MOVING_SPEED = 6; //* aceleracion de movimiento al ser comidos
var GHOST_AFFRAID_TIME = 8500; //* tiempo en su estado asustado
var GHOST_EAT_TIME = 5500; //* tiempo que estan fuera del juego (comidos)
var GHOST_BODY_STATE_MAX = 6;

//* funcion para llamar a los fantasmas
function initGhosts() { 
	initGhost('blinky');
	initGhost('pinky');
	initGhost('inky');
...
//* iniciar los fantasmas en el canvas
function initGhost(ghost) { 
	var canvas = document.getElementById('canvas-ghost-' + ghost);
	canvas.setAttribute('width', '550'); //* ancho de los fantasmas en el canvas
	canvas.setAttribute('height', '550'); //* alto de los fantasmas  en el canvas
	if (canvas.getContext) {  //* fantasmas en el canvas
		eval('GHOST_' + ghost.toUpperCase() + '_CANVAS_CONTEXT = canvas.getContext("2d")');
	}
}

//* restablecer los fantasmas (posicion, estado, movimiento, asustados, etc)
...
//* llamar a la funcion dibujar fantasmas
function drawGhosts() { 
	drawGhost("blinky");
	drawGhost('pinky');
	drawGhost('inky');
...
//* llamar a la funcion de los fantasmas asustados
...
//* estado de asustado de los fantasmas cancelado
...
//* estado de prueba de los fantasmas
function testStateGhosts() { 
	...
	if ( GHOST_BLINKY_STATE === 1 ||  
		...
		playWazaSound(); //* sonidos de los fantasmas
	} else if ( GHOST_BLINKY_STATE === -1 ||  
		 GHOST_PINKY_STATE === -1 ||  
		 GHOST_INKY_STATE === -1 ||  
		 GHOST_CLYDE_STATE === -1 
	) { 
		playGhostEatenSound();	//* sonidos de los fantasmas	
	} else { 
		playSirenSound(); //* sonidos de los fantasmas
	}
}

//* empezar a comer los fantasmas
function startEatGhost(ghost) { 
	
	if ( !LOCK ) { 
		playEatGhostSound();//* sonido fantasma comiendo
...
function eatGhost(ghost) { 
	...
	playGhostEatenSound(); //* sonido fantasma comido
...
//* cancelar estado comido del fantasma
...
//* llamar a la funcion mover fantasmas
function moveGhosts() { 
	moveGhost("blinky");
	moveGhost('pinky');
	moveGhost('inky');
...
//* funcion que mueve a los fantasmas
function moveGhost(ghost) {
	...
	if (eval('GHOST_' + ghost.toUpperCase() + '_MOVING === false')) { 
		...
		changeDirection(ghost); //* cambiar direccion del fantasma
		...
		//* donde puede moverse el fantasma
		if (canMoveGhost(ghost)) { 
			...
			drawGhost(ghost); //* dibujar fantasma
...
//* funcion del cambio de direccion del fantasma
function changeDirection(ghost) { 
	...
	var tryDirection = oneDirection(); //* una direccion
	...
	if (state === 0 || state === 1) { 
		...
		if (state === 1) { 
			tryDirection = reverseDirection(tryDirection); //* direccion contraria
		}
	} else { 
		var axe = oneAxe();
...
function getRightDirectionForHome(axe, ghostX, ghostY) {  //* direcion derecha de la casa
	var homeX = 276;
	var homeY = 204;
...
function getRightDirection(axe, ghostX, ghostY, pacmanX, pacmanY) {  //* direcion derecha
	if (axe === 1) { 
		if (ghostX > pacmanX) { 
		 return 3;
		} else { 
...
function canMoveGhost(ghost, direction) { //* donde se puede mover
	if (!direction) { 
		eval('var direction = GHOST_' + ghost.toUpperCase() + '_DIRECTION');
	}
...
//* funcion que para a los fantasmas
...
//* llamar a la funcion parar fantasmas
function stopGhosts() { 
	stopGhost('blinky');
	stopGhost('pinky');
	stopGhost('inky');
...
//* poner en pausa a los fantasmas
...
//* llamar a la funcion fantasmas en pausa
...
//* llamar a la funcion reanudar a los fantasmas
