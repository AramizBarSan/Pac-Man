...
function drawBoard(alternate) { 
	...
	if (alternate && alternate === true) { 
		...
		ctx.strokeStyle = "#193fff"; //*cambio el color del tablero 
	}
	
	ctx.lineWidth = "2"; //* grosor de las paredes 
	ctx.beginPath();
	
	// inicializacion del boton arriba 
	ctx.moveTo(0, 231);
	ctx.lineTo(97, 231);
	...
	// inicializacion del boton abajo 
	ctx.moveTo(0, 283);
	ctx.lineTo(97, 283);
	...
	// inicializacion del boton izquierda 
	...
	// inicializacion del boton derecha 
...
function drawBoardDoor() { 
	...
	ctx.strokeStyle = "white"; //* color de la puerta de los fantasmas 
	ctx.lineWidth = "5"; //* grosor de la linea
	
	ctx.beginPath();
	ctx.moveTo(255, 226); //* angulo de la puerta
	ctx.lineTo(295, 226); //* direccion de la puerta 
	ctx.stroke();
